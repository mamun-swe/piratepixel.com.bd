  ...
  return (
    <div>
      ...
      <Container.Fluid className="py-4">
        <Container.Row>
          <Container.Column>
            <p className="font-16 text-dark mb-0">
              Search for <span className="text-muted">{query}</span>
            </p>
            {loading && !data.length ? (
              <p className="font-14 text-muted mb-1">Searching...</p>
            ) : null}
            {!loading ? (
              <p className="font-14 text-muted mb-1">
                {CommaNumber(data.length ? data.length : 0)} Results found.
              </p>
            ) : null}
          </Container.Column>
        </Container.Row>
